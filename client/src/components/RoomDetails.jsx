...
export default function RoomDetails() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 sm:px-9">
      <Link to={`/hotels/${room.hotel}`} className="label mt-10 inline-block text-smoke no-underline hover:text-ink">
        ← {room.hotel_name}
      </Link>
      ...
      <dl className="mt-10 grid grid-cols-2 border-t-2 border-ink sm:grid-cols-4">
        ...
        <Fact label="Sleeps up to" value={`${room.max_guests} guests`} />
        <Fact label="Size" value={`${room.size} m²`} />
        <Fact label="Per night" value={`${room.price} ILS`} />
      </dl>

      <Link
        to={`/book?hotel=${room.hotel}&room=${room.id}`}
